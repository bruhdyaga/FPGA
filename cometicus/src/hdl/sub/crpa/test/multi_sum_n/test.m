...
% ==== Параметры модели, которые можно конфигурировать "свыше" ===
fid = fopen([TMP 'test_params.m'], 'w');

% Начальное состояние датчика случайных чисел
try, rand_state;
catch
    % Если нужны одинаковые результаты от запуска к запуску - раскомментировать строку ниже
    %    randn('state', 0);
    randn_state = randn('state');    
    % fprintf('randn state: %d %d\n', randn_state); % Выдача начального состояния датчика случайных чисел, чтобы можно было повторить
end
...
try, NS;
    ...
    NS = 17;                 % Число слагаемых
end
fprintf(fid, 'NS = %d;\n', NS);

try, IN_WIDTH;
catch
    IN_WIDTH = 46;             % Разрядность входных данных
end
fprintf(fid, 'IN_WIDTH = %d;\n', IN_WIDTH);

fclose(fid);  % Сохранение файла с параметрами
