...
int cpu_sim() {
    ...
    printf("AXI_TRIG SIZE_ID = 0x%08X\n", *axi_read(pBASE, 0, 0));
    
    axi_write(pBASE, 2, 0x40000000); // set BASE
    axi_write(pBASE, 3, 500); // set TIMEOUT
    axi_write(pBASE, 0, 1);   // RUN
    
    waitClks(1000);
    axi_write(pBASE, 1, 1);   // reset
    axi_write(pBASE, 2, 0x40000000); // set BASE
    axi_write(pBASE, 3, 2000); // set TIMEOUT
    axi_write(pBASE, 0, 1);   // RUN
